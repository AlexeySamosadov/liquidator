/**
 * Bridge USDT from BNB Chain to Arbitrum using Stargate
 */
...
async function main() {
    ...
    logger.info('Bridge USDT: BNB Chain → Arbitrum');
    logger.info('='.repeat(80));
    ...
    const amountToSend = ethers.parseUnits('30', 18); // 30 USDT

    if (balance < amountToSend) {
        logger.error(`Insufficient balance. Need 30 USDT, have ${ethers.formatUnits(balance, 18)}`);
        process.exit(1);
    }
